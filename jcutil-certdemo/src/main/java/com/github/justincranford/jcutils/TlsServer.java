...
public class TlsServer extends Thread {
	...
	public static void main(final String[] args) throws Exception {
		...
		final String       ksAlias         = CertChainDemo.KSALIASSERVER;
		final char[]       ksAliasPassword = new char[0];
		new TlsServer(secureRandom, hostAddress, port, soTimeout, tsFilePath, tsPassword, ksFilePath, ksPassword, ksAlias, ksAliasPassword).run(); // NOSONAR Call the method Thread.start() to execute the content of the run() method in a dedicated thread.
	}
	...
	public TlsServer(final SecureRandom secureRandom, final InetAddress hostAddress, final int port, final int soTimeout, final String tsFilePath, final char[] tsPassword, final String ksFilePath, final char[] ksPassword, final String ksAlias, final char[] ksAliasPassword) {
...
//			keyManagers[0] = new X509KeyManagerAppendTrustManager((X509KeyManager)keyManagers[0], (X509TrustManager)trustManagers[0], null);	// Wrap first KeyManager with a delegator object to append secondary trust chains from truststore
			...
			sslContext.init(keyManagers, (needClientAuth?trustManagers:null), this.secureRandom);
